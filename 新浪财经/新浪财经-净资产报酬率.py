...
def run():
    ...
    for stock in stocks:
        ...
        for year in years:
            url = 'http://money.finance.sina.com.cn/corp/go.php/vFD_FinancialGuideLine/stockid/' + str(stock[2:]) + '/ctrl/' + str(year) + '/displaytype/4.phtml'
            ...
            try:
                fuzhai_span = re.findall('净资产报酬率(.*?)资产', response.text)
                fuzhai = re.findall('<td>(.*?)<', fuzhai_span[0])
                fuzhai = float(str(fuzhai[0]).replace(',', ''))
                alist.append(float(fuzhai))
                print(fuzhai)
            except:
                alist.append('')
    ...
    info_table.to_csv('净资产报酬率.csv', encoding='gbk')
