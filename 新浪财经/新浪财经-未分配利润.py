...
def run():
    ...
    for stock in stocks:
        ...
        for year in years:
            ...
            try:
                lirun_span = re.findall('未分配利润(.*?)归属于', response.text)
                lirun = re.findall(';\'>(.*?)<', lirun_span[0])
                alist.append(float(str(lirun[0]).replace(',', '')))
            except:
                alist.append('')
    ...
    info_table.to_csv('未分配利润2009.csv', encoding='gbk')
