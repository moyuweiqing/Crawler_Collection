...
def run():
    ...
    for stock in stocks:
        ...
        for year in years:
            url = 'https://money.finance.sina.com.cn/corp/go.php/vFD_ProfitStatement/stockid/' + str(stock[2:]) + '/ctrl/' + str(year) + '/displaytype/4.phtml'
            ...
            try:
                fuzhai_span = re.findall('营业收入(.*?)二', response.text)
    ...
    info_table.to_csv('营业收入2009.csv', encoding='gbk')
