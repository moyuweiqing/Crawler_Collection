...
def run():
    ...
    for stock in stocks:
        ...
        for year in years:
            url = 'https://money.finance.sina.com.cn/corp/go.php/vFD_BalanceSheet/stockid/' + str(stock[2:]) + '/ctrl/' + str(year) + '/displaytype/4.phtml'
            ...
            # print(response.text)
            try:
                zichan_span = re.findall('资产总计(.*?)流动', response.text)
                zichan = re.findall(';\'>(.*?)<', zichan_span[0])
                zichan = float(str(zichan[0]).replace(',', ''))
                # alist.append(float(zichan))

                fuzhai_span = re.findall('负债合计(.*?)所有', response.text)
                fuzhai = re.findall(';\'>(.*?)<', fuzhai_span[0])
                fuzhai = float(str(fuzhai[0]).replace(',', ''))
                alist.append(float(fuzhai/zichan))
            except:
                alist.append('')
    ...
    info_table.to_csv('资产负债率.csv', encoding='gbk')
